...
func (oc *Controller) generatePodIPCacheForEgressIP(eIPs []interface{}) (map[string]sets.String, error) {
	...
	for _, eIP := range eIPs {
		if egressIP, ok := eIP.(*egressipv1.EgressIP); ok {
			egressIPToPodIPCache[egressIP.Name] = sets.NewString()
			if namespaces, err := oc.watchFactory.GetNamespacesBySelector(egressIP.Spec.NamespaceSelector); err == nil {
				for _, namespace := range namespaces {
					if pods, err := oc.watchFactory.GetPodsBySelector(namespace.Name, egressIP.Spec.PodSelector); err == nil {
						...
						klog.Errorf("Error building egress IP sync cache, cannot retrieve pods for namespace: %s, err: %v", namespace.Name, err)
					}
				}
			} else {
