	...
	Context("IPv6 on pod UPDATE", func() {
		...
		It("should remove OVN pod egress setup when EgressIP stops matching pod label", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				fakeOvn.fakeExec.AddFakeCmdsNoOutputNoError(
					[]string{
						fmt.Sprintf("ovn-nbctl --timeout=15 lr-policy-add ovn_cluster_router 100 ip6.src == %s reroute %s", egressPod.Status.PodIP, nodeGatewayRouterIP),
		...
		It("should treat pod update if pod did not have an assigned IP when it got the ADD", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				fakeOvn.fakeExec.AddFakeCmdsNoOutputNoError(
					[]string{
						fmt.Sprintf("ovn-nbctl --timeout=15 lr-policy-add ovn_cluster_router 100 ip6.src == %s reroute %s", podUpdate.Status.PodIP, nodeGatewayRouterIP),
	...
	Context("IPv6 on EgressIP UPDATE", func() {
		...
		It("should delete and re-create", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				eIPUpdate, err := fakeOvn.fakeEgressClient.K8sV1().EgressIPs().Get(context.TODO(), eIP.Name, metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				...
				_, err = fakeOvn.fakeEgressClient.K8sV1().EgressIPs().Update(context.TODO(), eIPUpdate, metav1.UpdateOptions{})
				Expect(err).ToNot(HaveOccurred())
				Eventually(fakeOvn.fakeExec.CalledMatchesExpected).Should(BeTrue(), fakeOvn.fakeExec.ErrorDesc)
				Eventually(status).Should(HaveLen(1))
				statuses = status()
