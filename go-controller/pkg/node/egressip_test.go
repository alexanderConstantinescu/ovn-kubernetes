...
var _ = Describe("EgressIP Operations", func() {
	...
	Context("in shared gateway mode on startup in IPv4", func() {
		...
		It("deletes stale egress IPs on current node", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				fakeOvnNode.start(ctx, []runtime.Object{
					&eIP,
				})

				fakeOvnNode.node.modeEgressIP = &egressIPShared{}
				fakeOvnNode.node.defaultGatewayIntf = primaryLinkName
				...
				err = fakeOvnNode.node.WatchEgressIP()
				Expect(err).NotTo(HaveOccurred())
	...
	Context("in shared gateway mode on ADD in IPv4", func() {
		...
		It("adds IPs with label", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				_, err = fakeOvnNode.fakeEgressClient.K8sV1().EgressIPs().Create(context.TODO(), &eIP, metav1.CreateOptions{})
	...
	Context("in shared gateway mode on DELETE in IPv4", func() {
		...
		It("deletes IPs with label", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				err = fakeOvnNode.fakeEgressClient.K8sV1().EgressIPs().Delete(context.TODO(), eIP.Name, *metav1.NewDeleteOptions(0))
				Expect(err).NotTo(HaveOccurred())
				Eventually(addrs).Should(HaveLen(1))
	...
	Context("in shared gateway mode on UPDATE in IPv4", func() {
		...
		It("adds and deletes IPs with label", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				_, err = fakeOvnNode.fakeEgressClient.K8sV1().EgressIPs().Update(context.TODO(), &eIPUPdate, metav1.UpdateOptions{})
				Expect(err).NotTo(HaveOccurred())
				Eventually(fakeOvnNode.fakeExec.CalledMatchesExpected, 3).Should(BeTrue(), fakeOvnNode.fakeExec.ErrorDesc)
				Eventually(addrs).Should(HaveLen(2))
	...
	Context("in local gateway mode on ADD in IPv4", func() {
		...
		It("adds IPs with label", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				fakeOvnNode.node.modeEgressIP = &egressIPLocal{
