...
func runOvnKube(ctx *cli.Context) error {
	...
	clientset, egressIPClientset, err := util.NewClientset(&config.Kubernetes)
	if err != nil {
		return err
	}

	// create factory and start the controllers asked for
	factory, err := factory.NewWatchFactory(clientset, egressIPClientset)
	if err != nil {
		return err
	}
	...
	if master != "" {
		...
		ovnController := ovn.NewOvnController(clientset, egressIPClientset, factory, stopChan, nil, ovnNBClient, ovnSBClient, util.EventRecorder(clientset))
